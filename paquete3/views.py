...
from paquete3.models import Cliente,Payment
from paquete3.forms import PaymentForm
from paquete3.forms import ClienteForm
...
def add_cliente(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cliente.save()
            return redirect('paquete3:cliente_list')
    else:
        form = ClienteForm()
...
#EN ESTE METODO INCLUYO PARTE DEL METODO PARA QUE DESAPAREZCA E CLIENTE SI LLEGA A 0
def cliente_list(request):
    if request.method == 'POST':
        cliente_id = request.POST.get('cliente_id')
        payment_amount = request.POST.get('payment_amount')
        move_to_saldo_cero = request.POST.get('move_to_saldo_cero')

        if move_to_saldo_cero:
            # Move all clients with saldo cero to a separate view
            return redirect('paquete3:clientes_con_saldo_cero')
        ...
        if cliente_id:
            ...
            if move_to_saldo_cero:
                ...
                cliente.saldo = 0
                cliente.save()
                return redirect('paquete3:cliente_list')  # Redirect to refresh the list
...
def edit_cliente(request, cliente_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('paquete3:cliente_list')  # Redirect to client list or detail view
    else:
...
def historial_de_pagos(request, cliente_id):
    ...
    if request.method == 'POST':
        ...
        if payment_id:
            ...
            return redirect('paquete3:client_detail', cliente_id=cliente_id)
...
def borrar_cliente(request, cliente_id):
    ...
    if request.method == 'POST':
        ...
        cliente.delete()
        return redirect('paquete3:cliente_list')  # Redirect to your client list URL name
...
from paquete3.forms import AgregarGastoForm,AgregarBaseForm
from paquete3.models import AgregarGasto,AgregarBase
...
def agregar_gasto(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('paquete3:lista_gastos_del_dia')  # Redirect to a page listing all gastos
    else:
...
def agregar_base(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('paquete3:historial_base')  # Redirect to a page listing all gastos
    else:
...
from paquete3.serializers import ClienteSerializer #ESTO ES PARA EL METODO DEL BUSCADOR EN TIEMPO REAL
